    ...
    <div id="screen-notifs" className="screen">
      <div className="fu">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
          <div>
            <div style={{ fontFamily: 'var(--hd)', fontWeight: 800, fontSize: 22, marginBottom: 4 }}>Notifications</div>
            <div style={{ color: 'var(--t3)', fontSize: 13 }}>{unread} unread</div>
          </div>
          ...
          {notifs.map((n) => (
            <div className={`notif-row ${n.read ? 'read' : ''}`} key={n.id} onClick={() => onRead(n.id)}>
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <div style={{ width: 38, height: 38, borderRadius: 9, background: n.read ? 'var(--s3)' : 'var(--org)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12.5, color: 'var(--t2)', lineHeight: 1.5 }}>{n.body}</div>
